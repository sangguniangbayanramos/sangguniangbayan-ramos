...
if (isset($_GET['search'])) {
  ...
  $query = "SELECT * FROM resolutions ORDER BY uploaded_at DESC";
}
...
<head>
  ...
  <title>Resolutions — Municipality of Ramos</title>
  <style>
    * {
...
<body>
  ...
  <main>
    ...
    <form class="search-container" method="GET">
      <input type="text" name="search" placeholder="Search Resolutions by number, title, or author..." value="<?= htmlspecialchars($search) ?>">
    ...
    <div class="grid">
      <?php if ($result && $result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <div class="card">
            <div class="year-tag"><?= date("Y", strtotime($row['uploaded_at'])) ?></div>

            <h3>Resolution No. <?= htmlspecialchars($row['number']) ?></h3>
        ...
        <?php endwhile; ?>
      <?php else: ?>
        <p style="text-align:center; color:#555;">No resolution uploaded yet.</p>
      <?php endif; ?>
    </div>
  </main>
